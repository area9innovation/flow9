...
function drawFrame(glContext, state) {
	...
	if (state.textures.length > 0) {
		...
		glContext.uniform1iv(glContext.getUniformLocation(state.shaderProgram, "textures"), state.textures.keys());
	}
...
function doResizeCanvas(glContext, state) {
	...
	glContext.bufferSubData(glContext.ARRAY_BUFFER, 0, vertices);
...
function doRecompileShader(glContext, state, fragCode) {
	let newFrag = glContext.createShader(glContext.FRAGMENT_SHADER);
	glContext.shaderSource(newFrag, fragCode); 
	glContext.compileShader(newFrag);

	let compilationLogF = glContext.getShaderInfoLog(newFrag);
	if (compilationLogF.length != 0) console.log('Fragment shader compiler log: ' + compilationLogF);
...
function rayMain(id) {
	...
	let glContext = state.canvas.getContext('webgl2');
	...
	glContext.bufferData(glContext.ARRAY_BUFFER, 32, glContext.STATIC_DRAW);
	...
	rayCanvasManager.set(id, {
		...
		loadTexture: (txtr) => {
			loadTexture(glContext, textureCounter++, txtr, saveTexture);
		},
		resetTextures: () => {
			state.textures.forEach((texture) => glContext.deleteTexture(texture));
			state.textures = [];
			textureBuffer.clear();
			textureCounter = 0;
		}
	});
	rayCanvasManagerCanvasResizeQueue.forEach(idToResize => {
		if (rayCanvasManager.has(idToResize)) {
			rayCanvasManager.get(idToResize).resizeCanvas()
			rayCanvasManagerCanvasResizeQueue = rayCanvasManagerCanvasResizeQueue.filter(v => v != idToResize)
		}
	});
...
function loadTexture(glContext, tid, url, saveTexture) {
