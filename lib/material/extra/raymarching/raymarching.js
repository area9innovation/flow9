...
function drawFrame(state, time) {
	...
	if (previousView != state.view && state.textPoints.length > 0) {
		...
		for(var i = 0; i < state.textPoints.length; i++) {
			var pointToCamera = glm.vec3(state.textPoints[i][0], state.textPoints[i][1], state.textPoints[i][2])['-'](state.cameraPosition);
			var rayDirection = glm.normalize(pointToCamera);
			var rayDirectionRotated = glm.inverse(state.view)['*'](glm.vec4(rayDirection, 1));
...
function initializeMouseEvents(state) {
	...
	canvas.addEventListener('mousemove', (evt) => {
		if (state.frameDrawn && (mouseLeftDown || mouseMiddleDown || mouseRightDown) && !state.firstPersonCamera && !state.onDemandRender) {
...
function rayMain(id) {
	...
	rayCanvasManager.set(id, {
		...
		doOnDemandRender: () => {if (state.onDemandRender) window.requestAnimationFrame((time) => drawLoop(state, time))},
