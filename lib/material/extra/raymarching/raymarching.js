...
function getDistance(p, invokeDistanceFn, positions, objectParameters, smoothCoefficients, spaces, repetitions) {
	const MAX_DIST = 1000;
	let d = {distance : MAX_DIST, id : -1};

	d = invokeDistanceFn(p, positions, objectParameters, smoothCoefficients, spaces, repetitions, d);

	return d;
}

function rayMarch(ro, rd, invokeDistanceFn, positions, objectParameters, smoothCoefficients, spaces, repetitions) {
	...
	for (let i=0; i< MAX_STEPS; i++){
		let p = ro['+'](rd['*'](dO));
		d = getDistance(p, invokeDistanceFn, positions, objectParameters, smoothCoefficients, spaces, repetitions);
		dO += d.distance;
...
function rayMain(id) {
	let state = {
		...
		invokeDistance: (p, positions, objectParameters, smoothCoefficients, spaces, repetitions, defaultDistance) => {return %distanceFunction%;},
		timeStart: 0,
		frameCount: 0,
