...
function drawLoop(state) {
	...
	window.requestAnimationFrame(() => drawLoop(state));
}
...
function rayMain(id) {
	...
	const uboBuffer = bindUBO(state, "TextureParamertersBlock", 0);
	const uboBuffer2 = bindUBO(state, "MaterialsBlock", 1);

	const uboVariableNames = ["color", "reflectiveness"];
	...
	rayCanvasManager.set(id, {
		...
		loadTextureParameters: (params) => {
			...
			gl.bufferSubData(gl.UNIFORM_BUFFER, 0, new Float32Array(params), 0);
		},
		updateMaterialsColor: (params) => {
			gl.bindBuffer(gl.UNIFORM_BUFFER, uboBuffer2);
			gl.bufferSubData(gl.UNIFORM_BUFFER, uboVariableInfo["color"].offset, new Float32Array(params), 0);
		},
		updateMaterialsReflectivness: (params) => {
			gl.bindBuffer(gl.UNIFORM_BUFFER, uboBuffer2);
			gl.bufferSubData(gl.UNIFORM_BUFFER, uboVariableInfo["reflectiveness"].offset, new Float32Array(params), 0);
		}
	});
