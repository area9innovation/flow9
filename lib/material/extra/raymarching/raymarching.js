...
function drawFrame(state, time) {
	...
	if (state.previousView != currentView && state.textPoints.length > 0) {
		...
		for(var i = 0; i < state.textPoints.length; i++) {
			...
			var rayDirectionRotated = glm.inverse(currentView)['*'](glm.vec4(rayDirection, 1));
		...
		window.postMessage(JSON.stringify({textPositions : buffer}));
	}
...
function initializeMouseEvents(state) {
	...
	canvas.addEventListener('mousemove', (evt) => {
		...
		if (state.blockMouse && blockMouse2) {
			state.previousView = state.cameraState.view;
...
function rayMain(id) {
	...
	rayCanvasManager.set(id, {
		...
		updateTextPoints: (textPoints) => state.textPoints = textPoints,
	});
