...
function getCameraRotationMatrix(state) {
	...
	let view = glm.rotate(glm.rotate(glm.mat4(1), yRotationAngle, glm.vec3(0, 1, 0)), xRotationAngle, glm.vec3(1, 0, 0));
	state.cameraState = {x : xRotationAngle, y : yRotationAngle, view : view, cameraPosition : cameraPosition, cameraLookAt : cameraLookAt};
	...
	if (previousView != view && state.textPoints.length > 0) {
		...
		for(var i = 0; i < state.textPoints.length; i++) {
			...
			var rayDirection = glm.normalize(pointToCamera);
			var rayDirectionRotated = glm.inverse(view)['*'](glm.vec4(rayDirection, 1));
			rayDirectionRotated = rayDirectionRotated['/'](rayDirectionRotated.z);
...
function sdHexagon(p, r)
{
    ...
    p['-='](glm.vec2(clamp(p.x, -k.z*r, k.z*r), r));
...
function sdTriangle(p, p0, p1, p2)
{
	...
	const clamp = (num, min, max) => Math.min(Math.max(num, min), max);
    let pq0 = v0['-'](e0['*'](clamp( glm.dot(v0,e0)/glm.dot(e0,e0), 0.0, 1.0)));
    let pq1 = v1['-'](e1['*'](clamp( glm.dot(v1,e1)/glm.dot(e1,e1), 0.0, 1.0)));
    let pq2 = v2['-'](e2['*'](clamp( glm.dot(v2,e2)/glm.dot(e2,e2), 0.0, 1.0)));
...
{    
    ...
    let c = a['*'](2.0);
    let d = A['-'](pos);
    let kk = 1.0/glm.dot(b,b);
    let kx = kk * glm.dot(a,b);
    let ky = kk * (2.0*glm.dot(a,a)+glm.dot(d,b)) / 3.0;
    let kz = kk * glm.dot(d,a);      
    let res = 0.0;
    let p = ky - kx*kx;
    let p3 = p*p*p;
    ...
    let h = q*q + 4.0*p3;
	const clamp = (num, min, max) => Math.min(Math.max(num, min), max);
	const dot2 = (vv) => glm.dot(vv, vv);
    if( h >= 0.0) 
    { 
        h = Math.sqrt(h);
       	let x = (glm.vec2(h - q,-h - q))['/'](2.0);
       	let uv = glm.sign(x)*powVec2(glm.abs(x), glm.vec2(1.0/3.0));
    ...
    else
    {
        ...
        let t = glm.clamp(subVec3(glm.vec3(m+m,-n-m,n-m)['*'](z), kx),0.0,1.0);
        res = Math.min( dot2(d['+']((c['+'](b['*'](t.x)))['*'](t.x))),
                   dot2(d['+']((c['+'](b['*'](t.y)))['*'](t.y))));
...
function getDistance(p, invokeDistanceFn, positions, objectParameters, smoothCoefficients, spaces, repetitions, shown) {
	const MAX_DIST = 1000;
	let d = {distance : MAX_DIST, id : -1};

	d = invokeDistanceFn(p, positions, objectParameters, smoothCoefficients, spaces, repetitions, shown, d);

	return d;
}

function rayMarch(ro, rd, invokeDistanceFn, positions, objectParameters, smoothCoefficients, spaces, repetitions, shown) {
	...
	for (let i=0; i< MAX_STEPS; i++){
		let p = ro['+'](rd['*'](dO));
		d = getDistance(p, invokeDistanceFn, positions, objectParameters, smoothCoefficients, spaces, repetitions, shown);
		dO += d.distance;
...
function initializeMouseEvents(state) {
	...
	let getMousePositionNormalized = (evt) => {
		...
		if (!allowFirstPersonMovement) uv = glm.vec2(
			(evt.clientX - boundingRect.x - 0.5 * boundingRect.width) / boundingRect.height,
			(0.5 * boundingRect.height - evt.clientY + boundingRect.y) / boundingRect.height
		);
		return glm.normalize(glm.vec3 (uv.x, uv.y, 1));
	}
	...
	let getRayDestination = (evt) => {
		let rd = getMousePositionNormalized(evt);
		rd = (state.cameraState.view['*'](glm.vec4(rd, 1))).xyz;
		return rd;
	}
...
function rayMain(id) {
	let state = {
		...
		invokeDistance: (p, positions, objectParameters, smoothCoefficients, spaces, repetitions, shown, defaultDistance) => {return %distanceFunction%;},
		timeStart: 0,
		frameCount: 0,
