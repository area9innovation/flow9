...
function rayMain(id) {
	...
	rayCanvasManager.set(id, {
		...
		updateMaterialsColor: (params) => {
			...
			gl.bufferSubData(gl.UNIFORM_BUFFER, uboVariableInfo["color[0]"].offset, new Float32Array(params), 0);
		},
		updateMaterialsReflectivness: (params) => {
			gl.bindBuffer(gl.UNIFORM_BUFFER, uboBuffer2);
			gl.bufferSubData(gl.UNIFORM_BUFFER, uboVariableInfo["reflectiveness[0]"].offset, new Float32Array(params), 0);
		}
	});
