...
function drawFrame(state) {
	...
	gl.bindBuffer(gl.UNIFORM_BUFFER, state.buffer);
	gl.bufferSubData(gl.UNIFORM_BUFFER, 0, state.textureParameters, 0);
...
function rayMain(id) {
	...
	const blockIndex = gl.getUniformBlockIndex(state.shaderProgram, "TextureParamertersBlock");
	...
	rayCanvasManager.set(id, {
		...
		loadTextureParameters: (params) => state.textureParameters = new Float32Array(params),
	});
