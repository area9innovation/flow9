...
function drawFrame(state, time) {
	...
	state.VAOs.forEach((e, i) => {
		...
		if (state.meshScales[i]) model = glm.scale(model, state.meshScales[i]);
		if (state.meshRotations[i]) {
			model = glm.rotate(model, state.meshRotations[i].x, glm.vec3(1, 0, 0));
			model = glm.rotate(model, state.meshRotations[i].y, glm.vec3(0, 1, 0));
			model = glm.rotate(model, state.meshRotations[i].z, glm.vec3(0, 0, 1));
		}
		if (state.meshPositions[i]) model = glm.translate(model, state.meshPositions[i]);
...
function rayMain(id) {
	...
	rayCanvasManager.set(id, {
		...
		updateMeshParameters: (positions, rotations, scales, colors) => {
			state.meshPositions = positions.map(p => glm.vec3(p[0], p[1], p[2]));
			state.meshRotations = rotations.map(p => glm.vec3(p[0], p[1], p[2]));
			state.meshScales = scales.map(p => glm.vec3(p[0], p[1], p[2]));
