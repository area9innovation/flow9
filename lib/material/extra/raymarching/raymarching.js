...
function initializeMouseEvents(state) {
	...
	getMousePositionNormalized = (evt) => {
		let uv = glm.vec2(0, 0);
...
function rayMain(id) {
	let state = {
		...
		timeStart: 0,
		frameCount: 0,
		uboVariableNames: ["color[0]", "reflectiveness[0]", "objectParameters[0]", "smoothCoefficients[0]", "repetitions[0]"],
	...
	rayCanvasManager.set(id, {
		...
		updateRepetitionCoefficients: (params) => {
			state.repetitionCoefficients = params;
			gl.bindBuffer(gl.UNIFORM_BUFFER, state.uboBufferObjectParameters);
			gl.bufferSubData(gl.UNIFORM_BUFFER, state.uboMaterialsInfo["repetitions[0]"].offset, new Float32Array(state.repetitionCoefficients), 0);
		},
	});
