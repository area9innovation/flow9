...
function doRecompileShader(state, fragCode) {
	...
	gl.bufferSubData(gl.UNIFORM_BUFFER, state.uboVariableInfo["color[0]"].offset, new Float32Array(state.colors), 0);
	gl.bufferSubData(gl.UNIFORM_BUFFER, state.uboVariableInfo["reflectiveness[0]"].offset, new Float32Array(state.reflectiveness), 0);
...
function rayMain(id) {
	...
	state.uboVariableInfo = {};
	uboVariableNames.forEach((name, index) => {
		state.uboVariableInfo[name] = {
