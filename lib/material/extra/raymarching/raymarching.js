...
function getDistance(p, invokeDistanceFn, positions) {
	const MAX_DIST = 1000;
	let d = {distance : MAX_DIST, id : -1};

	d = invokeDistanceFn(p, positions);

	return d;
}

function rayMarch(ro, rd, invokeDistanceFn, positions) {
	...
	for (let i=0; i< MAX_STEPS; i++){
		let p = ro['+'](rd['*'](dO));
		d = getDistance(p, invokeDistanceFn, positions);
		dO += d.distance;
...
function initializeMouseEvents(state) {
	...
	canvas.addEventListener('mousemove', function (evt) {
		...
		if (state.view !== undefined && state.positions !== undefined) {
			rd = (state.view['*'](glm.vec4(rd, 1))).xyz;
			let d = rayMarch(state.cameraPosition, rd, state.invokeDistance, getGLMPositions(state.positions));
...
function rayMain(id) {
	let state = {
		...
		invokeDistance: (p, positions) => {return %distanceFunction%;},
		timeStart: 0,
		frameCount: 0,
	};
