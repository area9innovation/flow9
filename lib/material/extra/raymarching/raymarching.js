...
function drawFrame(state, time) {
	...
	let projection = glm.perspective(glm.radians(55), state.canvas.width/state.canvas.height, 0.1, 2 * state.MAX_DIST);
	let view = glm.lookAt(state.cameraState.cameraPosition, state.cameraState.cameraLookAt, glm.vec3(0, 1, 0));
	...
	gl.uniformMatrix4fv(gl.getUniformLocation(state.shaderProgram, "view"), false, view.elements);
	...
	gl.uniformMatrix4fv(gl.getUniformLocation(state.shaderProgramMesh, "view"), false, view.elements);
	gl.uniformMatrix4fv(gl.getUniformLocation(state.shaderProgramMesh, "projection"), false, projection.elements);
...
function createMeshShader(id, state) {
	...
	let compilationLogF = gl.getShaderInfoLog(fragShader);
	...
	gl.attachShader(state.shaderProgramMesh, fragShader);
...
function rayMain(id) {
	...
	state.cubeVAO = gl.createVertexArray();
