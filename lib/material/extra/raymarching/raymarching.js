...
function getDistance(p, invokeDistanceFn, positions, objectParameters, smoothCoefficients) {
	...
	d = invokeDistanceFn(p, positions, objectParameters, smoothCoefficients);
...
function initializeMouseEvents(state) {
	...
	canvas.addEventListener('mousemove', (evt) => {
		...
		if (state.view !== undefined && state.positions !== undefined && state.objectParameters !== undefined) {
			...
			let d = rayMarch(state.cameraPosition, rd, state.invokeDistance, getGLMPositions(state.positions), getObjectParameters(state.objectParameters), getSmoothCoefficients(state.smoothCoefficients));
	...
	canvas.addEventListener('mousemove', (evt) => {
		if (state.frameDrawn && (mouseLeftDown || mouseMiddleDown || mouseRightDown) && !state.firstPerson) {
		...
		if (state.frameDrawn && state.firstPersonCamera && document.activeElement === canvas) {
			...
			rotateCamera = glm.rotate(rotateCamera, state.xRotationAngle + deltaY / 100., glm.vec3(1, 0, 0));
	...
	canvas.addEventListener('keydown', (evt) => {
		if (state.firstPersonCamera) {
			if (evt.code == 'KeyW' || evt.code == 'ArrowUp') {
				let direction = getCameraDirection(state);
				let move = glm.normalize(direction)['*'](0.5);
				state.cameraPosition['+='](glm.vec3(move.x, 0, move.z));
				state.cameraLookAt['+='](glm.vec3(move.x, 0, move.z));
			}
...
function rayMain(id) {
	let state = {
		...
		invokeDistance: (p, positions, objectParameters, smoothCoefficients) => {return %distanceFunction%;},
		timeStart: 0,
		frameCount: 0,
