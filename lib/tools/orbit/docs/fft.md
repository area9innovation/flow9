...
r ^ ((2*j)*k mod p) : NTTKernel →
	(r^2)^(j*k mod p) : SquaredBasePattern; // Note: Original superscript `r^2` kept for math readability
```
...
6.  **Automatic identity discovery**: The system can systematically explore algebraic properties (like twiddle factor identities) to discover the core relationships that enable optimization.
