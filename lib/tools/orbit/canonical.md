...
The alternating group A_n consists of all even permutations of n elements and is a subgroup of S_n.
...
// Reduction via known ideal (Groebner-like)
...
Au2099 u2282 Su2099;            // Au2099 is a subgroup of Su2099 of index 2
Cu2099 u2282 Du2099;            // Cu2099 is the rotation subgroup of Du2099
Cu2081 u2282 all groups;      // Trivial group is contained in all groups

// Normality relationships
Au2099 u23e2 Su2099;            // Au2099 is normal in Su2099 (unique subgroup of index 2)
Cu2099 u23e2 Du2099;            // Rotations preserved under conjugation in Du2099

// Quotient relationship
Su2099 / Au2099 u2245 Cu2082;        // Quotient group is cyclic of order 2

// Semi-direct product decomposition
Du2099 u2245 Cu2099 u22c4 Cu2082;       // Reflections acting on rotations
...
Su2099 is generated by (Cu2082)^n   // Transpositions generate Su2099

// Parity criterion
p u2208 Au2099 u21d4 number_of_transpositions(p) is even;  // Used to split Su2099

// Simplicity
Au2099 is simple for n u2265 5;   // No normal subgroups

// Commutativity
Cu2099 is abelian
Du2099 is non-abelian for n u2265 3
Su2099 is non-abelian for n u2265 3
Au2099 is non-abelian for n u2265 4

// Classical Group Relationships
GLu2099(Fu2091) u2283 SLu2099(Fu2091);      // Special linear is a subgroup of general linear
GLu2081(Fu2091) u2245 (Fu2091)* u2245 Cu2091u2088u2081;  // Multiplicative group of non-zero field elements
Zu2099 under addition u2245 Cu2099;    // Modular integers form a cyclic group
(Z/nZ)* is cyclic if φ(n) is cyclic;  // Multiplicative group of units
(Cu2082)^n is a vector space over GF(2);  // XOR operations form a vector space

// Quaternion Group Relations
Qu2088 is non-abelian of order 8;       // Not isomorphic to Du2084
Qu2088 has all non-identity elements of order 4;  // Different from Cu2084 and Cu2082 × Cu2084

// Group Products and Decompositions
Du2099 u2245 Cu2099 u22c4 Cu2082;              // Semidirect product with reflection acting on rotation
Su2099 u2245 Au2099 u22c4 Cu2082;              // Decomposition for n u2265 2
SLu2082(Fu2082) is simple for most fields;  // Special linear group of 2×2 matrices
```
...
#### Multiplication in (Z/2^nZ)* (Only valid when constants are odd)

```
// Inverse for multiplication
(x * k) * ku207bu00b9 => x : Canonical if gcd(k, 2^n) == 1;

// Associativity and folding
(x * ku2081) * ku2082 => x * eval(ku2081 * ku2082) : Canonical if gcd(ku2081, 2^n) == 1 && gcd(ku2082, 2^n) == 1;
```
...
#### Rotations in Cu2099 (Bit Rotation)
