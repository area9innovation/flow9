...
class JSBinflowBuffer {
	...
	private function doBinary(index : Int) : Dynamic {
		if (index < endIndex) {
			...
			if (word == 0xFFF4) {
				...
				var val = fixup == null ? NativeHx.makeStructValue(name, args[0], FlowIllegalStruct) : fixup(args[0]);
				return [val, args[1]];
			} else if (word == 0xFFF6) {
				// ref
				var v = doBinary(ni);
	...
	public function deserialise(defvalue : Dynamic, fixups : Dynamic) : Dynamic {
		if (FlowIllegalStruct == null) FlowIllegalStruct = NativeHx.makeStructValue("IllegalStruct", [], null);
