...
class RenderSupportJSPixiNew {
	...
	private static var RenderSupportJSPixiNewInitialised : Bool = init();
	...
	private static function workaroundDOMOverOutEventsTransparency() : Void {
		...
		var binder = function(fn) {
			return fn.bind(RenderSupportJSPixiNew.PixiRenderer.plugins.interaction);
		}
	...
	private static function createPixiRenderer() {
		...
		if (PixiRenderer != null) {
			...
			untyped __js__("delete RenderSupportJSPixiNew.PixiRenderer");
		}
		...
		if (PixiRenderer.plugins != null) {
			...
			untyped __js__("delete RenderSupportJSPixiNew.PixiRenderer.plugins.accessibility");
			untyped __js__("delete RenderSupportJSPixiNew.PixiRenderer.plugins.prepare");
		}
	...
	private static function initPixiRenderer() {
		...
		PixiRenderer.view.style.zIndex = RenderSupportJSPixiNew.zIndexValues.canvas;
	...
	public static function setAccessAttributes(clip : Dynamic, attributes : Array< Array<String> >) : Void {
		...
		if (clip.accessWidget == null) {
			...
			if (clip.nativeWidget != null) {
				...
				clip.accessWidget.style.zIndex = RenderSupportJSPixiNew.zIndexValues.accessButton;
				if (tagName == "button") {
	...
	public static function addFileDropListener(clip : Dynamic, maxFilesCount : Int, mimeTypeRegExpFilter : String, onDone : Array<Dynamic> -> Void) : Void -> Void {
		...
		dropArea.style.zIndex = RenderSupportJSPixiNew.zIndexValues.droparea;
		...
		var dropAreaProvideEvent = function (event : Dynamic) {
			event.preventDefault();
			dropArea.style.cursor = RenderSupportJSPixiNew.PixiRenderer.view.style.cursor;
			provideEvent(event);
		};
...
private class NativeWidgetClip extends FlowContainer {
	...
	public function updateNativeWidget() {
		...
		if (getClipVisible()) {
			...
			var tx = getClipWorldVisible() ? transform.tx : RenderSupportJSPixiNew.PixiRenderer.width;
			var ty = getClipWorldVisible() ? transform.ty : RenderSupportJSPixiNew.PixiRenderer.height;
	...
	private function addNativeWidget() : Void {
		if (nativeWidget != null) {
			if (parentNode == null) {
				parentNode = RenderSupportJSPixiNew.findParentAccessibleWidget(parent);
			}

			if (parentNode != null) {
				nativeWidget.style.position = "fixed";
				nativeWidget.style.zIndex = RenderSupportJSPixiNew.zIndexValues.nativeWidget;

				RenderSupportJSPixiNew.addNode(parentNode, nativeWidget);

				RenderSupportJSPixiNew.PixiStage.on("stagechanged", updateNativeWidget);
				once("removed", deleteNativeWidget);
			} else {
				RenderSupportJSPixiNew.findTopParent(this).once("added", addNativeWidget);
			}
		}
	}
	...
	private function deleteNativeWidget() : Void {
		RenderSupportJSPixiNew.PixiStage.off("stagechanged", updateNativeWidget);
		if (nativeWidget != null) {
	...
	public function requestFullScreen() : Void {
		if (nativeWidget != null) {
			RenderSupportJSPixiNew.requestFullScreen(nativeWidget);
		}
	}

	public function exitFullScreen() : Void {
		if (nativeWidget != null) {
			RenderSupportJSPixiNew.exitFullScreen(nativeWidget);
		}
	}
}
...
private class VideoClip extends FlowContainer {
	...
	private function createVideoClip(filename : String, startPaused : Bool) : Void {
		...
		RenderSupportJSPixiNew.PixiStage.on("drawframe", updateNativeWidget);
	...
	private function deleteVideoClip() : Void {
		if (nativeWidget != null) {
			...
			RenderSupportJSPixiNew.PixiStage.off("drawframe", updateNativeWidget);
	...
	private function createSubtitlesClip() : Void {
		if (textField == null) {
			textField = RenderSupportJSPixiNew.makeTextField(fontFamily);
			addChild(textField);
		};
	}
	...
	private function onMetadataLoaded() {
		...
		metricsFn(nativeWidget.width / RenderSupportJSPixiNew.backingStoreRatio, nativeWidget.height / RenderSupportJSPixiNew.backingStoreRatio);

		checkTimeRange(nativeWidget.currentTime, true);

		RenderSupportJSPixiNew.InvalidateStage(); // Update the widget
	...
	private function createFullScreenListeners() {
		if (nativeWidget != null) {
			if (Platform.isIOS) {
				nativeWidget.addEventListener('webkitbeginfullscreen', RenderSupportJSPixiNew.fullScreenTrigger, false);
				nativeWidget.addEventListener('webkitendfullscreen', RenderSupportJSPixiNew.fullScreenTrigger, false);
			}

			nativeWidget.addEventListener('fullscreenchange', RenderSupportJSPixiNew.fullScreenTrigger, false);
			nativeWidget.addEventListener('webkitfullscreenchange', RenderSupportJSPixiNew.fullScreenTrigger, false);
			nativeWidget.addEventListener('mozfullscreenchange', RenderSupportJSPixiNew.fullScreenTrigger, false);
		}
	}
	...
	private function destroyFullScreenListeners() {
		if (nativeWidget != null) {
			if (Platform.isIOS) {
				nativeWidget.removeEventListener('webkitbeginfullscreen', RenderSupportJSPixiNew.fullScreenTrigger);
				nativeWidget.removeEventListener('webkitendfullscreen', RenderSupportJSPixiNew.fullScreenTrigger);
			}

			nativeWidget.removeEventListener('fullscreenchange', RenderSupportJSPixiNew.fullScreenTrigger);
			nativeWidget.removeEventListener('webkitfullscreenchange', RenderSupportJSPixiNew.fullScreenTrigger);
			nativeWidget.removeEventListener('mozfullscreenchange', RenderSupportJSPixiNew.fullScreenTrigger);
		}
	}
}
...
private class WebClip extends NativeWidgetClip {
	...
	private function appendReloadBlock() : Void {
		...
		untyped img.src = "http://localhost/flow/images/lms_reload.png";
		div.appendChild(img);
...
private class TextField extends NativeWidgetClip {
	...
	private override function createNativeWidget(nodeName : String) : Void {
		...
		if (Platform.isIE || Platform.isEdge)
			RenderSupportJSPixiNew.PixiStage.on("preonfocus", preOnFocus);
	}

	private override function deleteNativeWidget() : Void {
		if (Platform.isIE || Platform.isEdge)
			RenderSupportJSPixiNew.PixiStage.off("preonfocus", preOnFocus);
	...
	private function updateNativeWidgetStyle() {
		if (isInput()) {
			...
			nativeWidget.style.color = RenderSupportJSPixiNew.makeCSSColor(fillColor, fillOpacity);
			nativeWidget.style.letterSpacing = (RenderSupportJSPixiNew.UseDFont ? letterSpacing + 0.022 : letterSpacing) + "px";
			...
			nativeWidget.style.backgroundColor = RenderSupportJSPixiNew.makeCSSColor(backgroundColor, backgroundOpacity);
			nativeWidget.style.cursor = "text";
			nativeWidget.style.opacity = preFocus && multiline && Platform.isEdge ? 1 : fillOpacity * worldAlpha;
			if (cursorColor >= 0) {
				nativeWidget.style.caretColor = RenderSupportJSPixiNew.makeCSSColor(cursorColor, cursorOpacity);
			}
	...
	private function onMouseMove(e : js.html.MouseEvent) {
		...
		nativeWidget.style.cursor = RenderSupportJSPixiNew.PixiRenderer.view.style.cursor;
		RenderSupportJSPixiNew.provideEvent(e);
	}
	...
	private function onMouseDown(e : Dynamic) {
		...
		nativeWidget.readOnly = shouldPreventFromFocus = RenderSupportJSPixiNew.getClipAt(point) != this;
	...
	private function onKeyDown(e : Dynamic) {
		if (TextInputKeyDownFilters.length > 0) {
			var ke : Dynamic = RenderSupportJSPixiNew.parseKeyEvent(e);

			for (f in TextInputKeyDownFilters) {
				if (!f(ke.key, ke.ctrl, ke.shift, ke.alt, ke.meta, ke.keyCode)) {
					ke.preventDefault();
					RenderSupportJSPixiNew.PixiStage.emit("keydown", ke);
					break;
				}
			}
		}
	...
	private function onKeyUp(e : Dynamic) {
		...
		if (TextInputKeyUpFilters.length > 0) {
			...
			for (f in TextInputKeyUpFilters) {
				if (!f(ke.key, ke.ctrl, ke.shift, ke.alt, ke.meta, ke.keyCode)) {
					ke.preventDefault();
					RenderSupportJSPixiNew.PixiStage.emit("keyup", ke);
					break;
				}
			}
		}
...
private class PixiText extends TextField {
		...
		style =
			{
				...
				fill : "#" + StringTools.hex(RenderSupportJSPixiNew.removeAlphaChannel(fillcolor), 6),
	...
	private function createTextClip(text : String, style : Dynamic) : Text {
		...
		if (RenderSupportJSPixiNew.AntialiasFont && (Platform.isChrome || Platform.isSafari)) {
...
private class DFontText extends TextField {
	...
	private override function makeTextClip(text : String, style : Dynamic) : Dynamic {
		...
		clip.resolution = RenderSupportJSPixiNew.PixiRenderer.resolution; // calculateBounds needs that
