...
/*
		...
		var on_ime_composition = function(e) {
			...
			NativeHx.println("on_ime_composition! ");
		};
		...
		try {
			...
			NativeHx.println("crash " + e);
		}

		//flash.system.IME.conversionMode = flash.system.IMEConversionMode.CHINESE;
		NativeHx.println("supported " +  flash.system.IME.isSupported);
		NativeHx.println("enabled " +  flash.system.IME.enabled);
		NativeHx.println("has ime " +  flash.system.Capabilities.hasIME);
		NativeHx.println("conversion mode " + flash.system.IME.conversionMode);
*/
		#end
	}
	...
	private static function addPasteClipboardListener(clip : Dynamic) {
		var pasteFromClipboard = function(e)  {
			if(Clipboard.generalClipboard.hasFormat(ClipboardFormats.TEXT_FORMAT)) {
                NativeHx.clipboardData = Clipboard.generalClipboard.getData(ClipboardFormats.TEXT_FORMAT);
            } else if(Clipboard.generalClipboard.hasFormat(ClipboardFormats.HTML_FORMAT)) {
                NativeHx.clipboardData = Clipboard.generalClipboard.getData(ClipboardFormats.HTML_FORMAT);
            } else {
			...
			if (event == "keydown" || event == "keyup") {
				...
				var keycb = function(e : flash.events.KeyboardEvent) {
					...
					try {
						...
						NativeHx.println("FATAL ERROR: Key handler reported: " + crashInfo);
						Assert.printStack(e);
						NativeHx.callFlowCrashHandlers("[KeyEvent Handler]: " + crashInfo);
					}
				}
	...
	public static function addEventListener(clip : Dynamic, event : String, fn : Void -> Void) : Void -> Void {
			...
			var cb = function(e) {
				try {
					...
					NativeHx.println("FATAL ERROR: Event handler reported: " + crashInfo);
					Assert.printStack(e);
					NativeHx.callFlowCrashHandlers("[Event Handler]: " + crashInfo);
				}
			};
			var eventname = event;
			if (event == "click") {
	...
	public static function getNumberOfCameras() : Int {
		#if js
		#elseif flash
			return CameraHx.getNumberOfCameras();
		#end
	  return 0;
	}
...
private class Graphics {
	...
	private function addSVGGradient(svg : Dynamic, id : String) {
		...
		for (i in 0...fillGradientColors.length) {
			var stop_pt = createSVGElement("stop", [
				...
				{n : "stop-color", v : RenderSupportHx.makeCSSColor(fillGradientColors[i], fillGradientAlphas[i]) }
			]);
	...
	private function renderSVG() {
		...
		if (strokeOpacity != 0.0)
			svgpath_attr.push({n : "stroke", v : RenderSupportHx.makeCSSColor(strokeColor, strokeOpacity)});

		if (fillOpacity != 0.0) {
			svgpath_attr.push({n : "fill", v : RenderSupportHx.makeCSSColor(fillColor, fillOpacity)});
		} else if (fillGradientColors != null) {
			var id = "grad" + Date.now().getTime();
			addSVGGradient(svg, id);
			svgpath_attr.push({n : "fill", v : "url(#" + id + ")"});
		} else {
			svgpath_attr.push({n : "fill", v : RenderSupportHx.makeCSSColor(0xFFFFFF, 0.0)});
		}
	...
	private function renderCanvas() {
		...
		if (strokeOpacity != 0.0) {
			...
			ctx.strokeStyle = RenderSupportHx.makeCSSColor(strokeColor, strokeOpacity);
		}

		if (fillOpacity != 0.0) {
			ctx.fillStyle = RenderSupportHx.makeCSSColor(fillColor, fillOpacity);
		}
		...
		if (fillGradientColors != null) {
			...
			for (i in 0...fillGradientColors.length)
				gradient.addColorStop(fillGradientOffsets[i], RenderSupportHx.makeCSSColor(fillGradientColors[i], fillGradientAlphas[i]) );
