...
template <typename A>
void dupVectItem(std::vector<A>& flow_a, std::size_t& i) {
	int32_t isUsed = isObjectUsed(flow_a[i - 1]); // cntr_flow_a[i - 1]
	std::cout<<"dupVectItem: i="<< i << "; " << (!isUsed ? " :: free value " : " :: value is used ") << std::endl;
...
// TODO: recursive check. simple types (int, double, string). variant ?
template <typename A>
int32_t isObjectUsed(A& val) {
	return val._counter > 0;
}

template <typename A>
void dupVectCounter(std::vector<A>& flow_a) {
	bool isUsed = false;
	for (std::size_t i = 0; i != flow_a.size(); ++i) {
		isUsed = isObjectUsed(flow_a[i]);
		if (isUsed) break;
	}
	std::cout<<"dupVectCounter: " << (!isUsed ? "free values " : "values are used. Counter=") << (!isUsed ? "" : (flow_print2(flow_a), "")) << std::endl;
	if (!isUsed) {
...
std::vector<B> gc_flow_map(std::vector<A>& flow_a, const std::function<B(A&)> & flow_fn) {
	...
	for (std::size_t i = flow_a.size(); i > 0; ) {
		res[i-1/*res.size() - i*/] = flow_fn(flow_a[i - 1]);
		dupVectItem(flow_a, i);
	}
	
	dupVectCounter(flow_a);
