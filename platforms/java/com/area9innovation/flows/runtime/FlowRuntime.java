package com.area9innovation.flow.runtime;
...
public abstract class FlowRuntime {
	...
	public static void initStrArgs(String[] args) {
		str_args = new Object[args.length][2];
		...
		for (int i = 0; i < args.length; i++) {
			...
			int pos = p.indexOf("=");
			if (pos >= 0) {
				str_args[i][0] = p.substring(0, pos);
				str_args[i][1] = p.substring(pos + 1);
			} else {
				str_args[i][0] = p;
				str_args[i][1] = "";
			}
...
*/
	...
	public static final Struct makeStructValue(String name, Object[] fields, Struct default_value) {
		Integer id = FlowStaticRuntime.struct_ids.get(name);
		if (id == null)
	...
	public static final Struct makeStructValue(int id, Object[] fields, Struct default_value) {
		try {
			Struct copy = FlowStaticRuntime.struct_prototypes[id].clone();
	...
	public static String[] getUrlArgs() {
		return FlowStaticRuntime.str_args;
	}
}
